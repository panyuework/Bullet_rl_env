...
import numpy as np
from Env.envs.envs import bases
...
class RaceCar(bases.Agent):
    ...
    def _set_car_constraints(self):
        ...
        """
        car = self.body_id
        for wheel in range(pb.getNumJoints(car)):
            pb.setJointMotorControl2(car,
                                          wheel,
                                          pb.VELOCITY_CONTROL,
                                          targetVelocity=0,
                                          force=0)
            pb.getJointInfo(car, wheel)

        c = pb.createConstraint(car,
                                     9,
                                     car,
                                     11,
        ...
        pb.changeConstraint(c, gearRatio=1, maxForce=10000)
        ...
        c = pb.createConstraint(car,
                                     ...
                                     parentFramePosition=[0, 0, 0],
                                     childFramePosition=[0, 0, 0])
        pb.changeConstraint(c, gearRatio=-1, gearAuxLink=15, maxForce=10000)
        c = pb.createConstraint(car,
                                     3,
                                     car,
                                     19,
...
class Drone(bases.Agent):
    ...
    def __init__(self,
                 bc,
                 init_xyz=(0, 0, 2),  # initially touches ground plane
                 debug=False):
        # from URDF file..
        self.radius = 0.14
        ...
        for m in self.motor_list:
            pb.changeDynamics(self.body_id, m.index,
    ...
    def apply_action(self, motorCommands):
        ...
        xyz, abcd = pb.getBasePositionAndOrientation(self.body_id)
        xyz_dot, rpy_dot = pb.getBaseVelocity(self.body_id)

        # matrix of size 3x3
        R = np.array(pb.getMatrixFromQuaternion(abcd)).reshape((3, 3))
...
class Drone2(bases.Agent):
    ...
    def __init__(self,
                 bc,
                 init_xyz=(-2, 0, 2),  # initially touches ground plane
                 debug=False):
        # from URDF file..
        self.radius = 0.14
        self.diameter = 2. * self.radius
