...
import numpy as np
import abc
from Env.envs.envs import sensors
...
class Joint(object):
    ...
    def __init__(
        ...
        jointInfo = pb.getJointInfo(body_id, joint_index)
    ...
    def get_torque(self) -> float:
        ...
        *_, applied_torque = pb.getJointState(
            self.body_id,
            self.index)
...
class Link(object):
    ...
    def get_state(self) -> np.ndarray:
        ...
        if self.index == -1:
            ...
            xyz, abcd, _, _, _, _, xyz_dot, rpy_dot = pb.getLinkState(
...
class Body(object):
    ...
    def __init__(
        ...
        self.num_joints = pb.getNumJoints(self.body_id)
...
class Agent(Body):
    ...
    def __init__(
        ...
        part_name, robot_name = pb.getBodyInfo(self.body_id)
        ...
        self.violation_shape = pb.createVisualShape(
            pb.GEOM_SPHERE,
            radius=self.size_violation_shape,
            rgbaColor=[1, 0, 0, 0.001],  # alpha = 0 renders shadows
            specularColor=[0, 0, 0],
        )
        self.violation_body_id = pb.createMultiBody(
            baseMass=0,
