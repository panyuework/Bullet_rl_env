...
class x10car_v1:
    def __init__(self, client):
        self.client = client
        f_name = os.path.join(os.getcwd(), 'Env/resources/x10car_v1.urdf')
        self.x10car_v1 = p.loadURDF(fileName=f_name,
                              basePosition=[0.0,0,0.05], #baseOrientation=p.getQuaternionFromEuler([0,0,-90*math.pi/180]),
    ...
    def get_ids(self):
        return self.x10car_v1, self.client
    ...
    def apply_action(self, action):
        ...
        p.setJointMotorControlArray(self.x10car_v1, self.steering_joints,
    ...
    def get_observation(self):
        ...
        pos, ang = p.getBasePositionAndOrientation(self.x10car_v1, self.client)
        ang = p.getEulerFromQuaternion(ang)
        ori = (math.cos(ang[2]), math.sin(ang[2]))
        #pos = pos[:2]
        # Get the velocity of the car
        vel = p.getBaseVelocity(self.x10car_v1, self.client)[0][0:2]
    ...
    def get_distance(self):
        ...
        pos, ang = p.getBasePositionAndOrientation(self.x10car_v1, physicsClientId=self.client)
        ...
        for i in range(numRays):
            ...
            rayTo.append([
                lidarPosition[0] + rayLen * math.cos(lidarOrientation  + rayFoV * float(i) / numRays),
                lidarPosition[1] + rayLen * math.sin(lidarOrientation + rayFoV * float(i) / numRays), lidarPosition[2] + 0.2525
            ])
    ...
    def get_camImg(self, car_id, client_id, path):
        if self.rendered_img is None:
            self.rendered_img = plt.imshow(np.zeros((100, 100, 4)))

        # Base information
        #car_id, client_id = self.racecar.get_ids()
        ...
        view_matrix = p.computeViewMatrix([pos[0]+0.35, pos[1]+0, pos[2]+0.08], pos + camera_vec, up_vec)
